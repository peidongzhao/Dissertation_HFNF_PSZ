...
for j in miu:
    ...
    for i in beta:  
        ...
        RE = FractureStress(half_length,S1,S2,P_net,i,j,P_pore,delta,x_wide,y_wide)
        area[num_beta] = RE.area
        num_beta += 1 
               
    DC = 'C'+str(num_S_diff)
    
    plt.plot(beta,area,color=DC,label='$\\mu$  = %.1f'%j)
